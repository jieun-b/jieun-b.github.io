...
ControlNet은 대규모의 사전 학습된 text-to-image diffusion 모델에 대한 conditional control을 학습하는 end-to-end 신경망 구조이다. ControlNet은 모델의 가중치들을 잠그고, 인코딩 레이어의 trainable copy를 만들어 대형 모델의 성능을 보존한다. 이러한 구조는 다양한 조건부 제어 학습을 위해 사전 학습된 대형 모델을 강력한 backbone으로 취급한다.

Trainable copy와 locked 모델은 가중치가 0으로 초기화된 zero convolution layer로 연결된다. 컨볼루션 가중치들은 학습 동안 점진적으로 성장하게 된다. 이는 학습 초기에 모델의 deep feature에 해로운 노이즈가 추가되지 않도록 보장하고, trainable copy에서 노이즈에 의한 손상에서 large-scale pretrained backbone을 보호한다.
...
ControlNet은 신경망 블록에 추가적인 조건을 주입한다. 이때 네트워크 블록은 신경망을 구성하는 레이어의 집합을 의미한다.

$\cal F(\cdot;\Theta)$을 파라미터 $\Theta$를 가지는 학습된 neural block이라고 가정해보자. $\boldsymbol{x} ∈ \mathbb{R} ^{h×w×c}$를 만족하는 2D feature map $\boldsymbol{x}$가 있을 때, $\cal F(\cdot;\Theta)$는 $\boldsymbol{x}$를 다른 feature map $\boldsymbol{y}$로 변환하게 된다. 이는 그림(a)와 같이 시각화 할 수 있으며, 식으로는 다음과 같이 나타낼 수 있다.

![img3.png](/assets/img/2023-08-29/img3.png){: width="60%" height="60%"}

이러한 사전 학습된 neural block에 ControlNet을 추가하기 위해서는 기존 블록의 매개변수 $\Theta$를 잠그고, 동시에 매개변수 블록을 trainable copy로 복사한다. 이 복사본의 매개변수는 $\Theta_c$로 표시된다. Trainable copy는 외부 조건 벡터 $\boldsymbol{c}$를 입력으로 가지게 된다.

Stable Diffusion 같은 대형 모델에 이를 적용하면, locked parameter는 production-ready model을 보존하며, 동시에 traninable copy는 그러한 사전 훈련된 대형 모델을 재사용해 다양한 입력 조건을 다루기 위한 깊고 강력한 백본을 구축한다.

Trainable copy는 $\cal Z(\cdot;\cdot)$로 표현되는 zero convolution layer를 통해 locked model에 연결된다. 제로 컨볼루션 레이어는 weight와 bias가 모두 0으로 초기화된 1$×$1 컨볼루션 레이어이다. ControlNet을 빌드업 하기 위해 파라미터 $\Theta_{z1}$와 $\Theta_{z2}$를 갖는 두 개의 제로 컨볼루션 인스턴스를 사용한다. ControlNet은 최종적으로 다음 식을 계산하게 된다.

![img4.png](/assets/img/2023-08-29/img4.png){: width="60%" height="60%"}

첫 번째 학습 단계에서 제로 컨볼루션 레이어의 가중치와 편향은 0으로 초기화되기 때문에, 위 식의 $\cal Z(\cdot;\cdot)$항들이 모두 0으로 계산되며, $\boldsymbol{y_c}=\boldsymbol{y}$가 성립하게 된다. 이를 통해 학습이 시작될 때, 해로운 노이즈가 trainable copy에서 신경망 레이어의 hidden state에 영향을 미치지 못하도록 한다. 

또한, $\cal Z(\boldsymbol{c};\Theta_{z1})=0$이고 trainable copy는 입력 이미지 $\boldsymbol{x}$를 받기 때문에 trainable copy는 완전히 기능하며 사전 훈련된 대형 모델의 능력을 유지하고 추가 학습을 위한 강력한 백본 역할을 한다. 제로 컨볼루션은 초기 학습 단계에서 그래디언트로 랜덤 노이즈를 제거함으로써 백본을 보호한다.

## ControlNet for Text-to-Image Diffusion

ControlNet이 large pretrained diffusion model에 조건부 제어를 추가하는 방법을 설명하기 위해 Stable Diffusion을 예시로 활용한다.

Stable Diffusion은 encoder, middle block, skip-connected decoder를 포함하는 U-Net 구조의 형태를 가지고 있다. 모델은 전체 25개 블록으로 구성되어 있는데, 인코더와 디코더 각각 12개 블록을 포함하며 하나의 중간 블록이 있다. 25개 블록 중 8개의 블록은 다운 샘플링 또는 업 샘플링 컨볼루션 레이어이며, 다른 17개 블록은 각각 4개의 resnet 레이어와 2개의 ViT를 포함하는 메인 블록이다. 
...
이렇게 ControlNet을 연결하는 방식은 기존 파라미터를 잠가 파인튜닝을 위 기존의 잠금된 인코더에서 그래디언트 계산이 필요하지 않게 된다. 이는 학습 속도를 높이고 GPU 메모리를 절약하는데 도움이 된다.
...
$\cal{L}$은 전체 diffusion model의 learning objective이며, ControlNet을 통해 diffusion 모델을 finetuning하는 데 직접 사용된다.

학습 중에, 저자들은 랜덤하게 텍스트 프롬프트 $$\boldsymbol{c}_\mathrm{t}$$의 $$50\%$$를 빈 문자열로 바꾼다. 이는 입력 조건 맵에서 semantic content를 인식하는 ControlNet의 능력을 향상시킨다. 

학습하는 동안 제로 컨볼루션이 네트워크에 노이즈를 추가하지 않기 때문에 모델은 항상 고품질 이미지를 예측할 수 있다. 모델은 점진적으로 제어 조건을 학습하는 것이 아니라 갑자기 입력 조건 이미지를 따라가게 된다. 이러한 현상은 sudden convergence phenomenon이라 부르며, 다음 그림에서 확인할 수 있다.
...
![img10.png](/assets/img/2023-08-29/img10.png)
