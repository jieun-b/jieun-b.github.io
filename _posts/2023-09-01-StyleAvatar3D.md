...
ControlNet $\mathscr{C}_\theta$ 은 pose image $I_\mathrm{p}$와 text prompt $T$를 입력으로 받아 스타일화된 이미지 $I_s:I_s=\mathscr{C}_\theta(I_\mathrm{p},T)$ 를 생성한다. 텍스트 프롬프트 $T$는 positive 프롬프트와 negative 프롬프트로 구성되어있다: $T=(T_\mathrm{pos},T_\mathrm{neg})$. 두 프롬프트는 각각 이미지에서 원하는 특성과 원하지 않는 특성으로 설명될 수 있다.
...
먼저 모든 렌더링 뷰를 yaw와 pitch 값을 기반으로 $N_\bold{group}$개의 그룹으로 나눈다. 각 그룹에는 one-hot yaw representation과 one-hot pitch representation이 할당된다. fine pose label을 얻기 위해 큰 그룹 번호를 할당하고, 작은 그룹 번호는 coarse pose label을 얻기 위해 할당된다. 두 유형의 라벨은 yaw와 pitch의 one-hot 표현을 연결(concatenate)해 표현된다. 판별자에서 사용되는 최종 포즈 라벨은 fine label과 coarse label을 연결하여 형성된다. $c=c_\mathrm{fine}||c_\mathrm{coarse}$. 
...
Diffusion model의 목표는 렌더된 정면 이미지를 가이드로 노이즈로부터 스타일 벡터를 diffuse하는 것이다. Diffusion model $\bm{\epsilon}_\theta$에는 PriorTransformer를 사용하는데, 노이즈 스타일 벡터 $\boldsymbol{w}$와 정면 이미지의 CLIP 임베딩 $\boldsymbol{y}$를 입력으로 받아 노이즈 $\boldsymbol{\epsilon}$을 예측한다. 

![img5.png](/assets/img/2023-09-01/img5.png)
...
![img7.png](/assets/img/2023-09-01/img7.png)
